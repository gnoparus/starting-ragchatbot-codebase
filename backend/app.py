...
app_logger.info(
    "Initializing RAG system with config",
    config={
...
async def query_documents(request: QueryRequest, http_request: Request):
    ...
    try:
        ...
        if not session_id:
            ...
            api_logger.debug(
                "Created new session", request_id=request_id, session_id=session_id
            )
...
async def get_course_stats(http_request: Request):
    ...
    api_logger.info(
        "API Course Stats Request", request_id=request_id, client_ip=client_ip
    )
