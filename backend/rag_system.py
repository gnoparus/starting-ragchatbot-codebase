...
class RAGSystem:
    ...
    def __init__(self, config):
        log.info(
            ...
            config_max_results=config.MAX_RESULTS,
        )
    ...
    def add_course_document(self, file_path: str) -> Tuple[Course, int]:
        ...
        try:
            ...
            log.debug(
                "Processing document with document processor", file_path=file_path
            )
            ...
            if not course:
                log.warning(
                    "No course data extracted from document", file_path=file_path
                )
                return None, 0
            ...
            log.debug(
                "Adding course metadata to vector store", course_title=course.title
            )
            self.vector_store.add_course_metadata(course)

            # Add course content chunks to vector store
            log.debug(
                "Adding course content chunks to vector store",
                chunks_count=len(course_chunks),
            )
    ...
    def query(
        ...
        if session_id:
            ...
            if history:
                log.debug(
                    f"Found {len(history)} previous exchanges in history",
                    session_id=session_id,
                )

        # Generate response using AI with tools
        log.debug(
            "Generating AI response with tools",
            available_tools=len(self.tool_manager.get_tool_definitions()),
        )
    ...
    def get_course_analytics(self) -> Dict:
        ...
        try:
            ...
            analytics = {"total_courses": total_courses, "course_titles": course_titles}
