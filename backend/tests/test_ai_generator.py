...
class TestAIGenerator:
    ...
    def test_generate_response_with_tools_no_tool_use(
        self, mock_anthropic_class, mock_anthropic_client, sample_tool_definitions
    ):
    ...
    def test_generate_response_with_tool_use(
        ...
        result = generator.generate_response(
            query, tools=sample_tool_definitions, tool_manager=mock_tool_manager
        )
    ...
    def test_handle_tool_execution_single_tool(
        self, mock_tool_use_response, sample_tool_definitions
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_tool_manager_none_handling(
        self, mock_anthropic_class, mock_tool_use_response, sample_tool_definitions
    ):
        ...
        result = generator.generate_response(
            "test query", tools=sample_tool_definitions, tool_manager=None
        )
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_message_structure_without_history(
        self, mock_anthropic_class, mock_anthropic_client
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_system_message_without_history(
        self, mock_anthropic_class, mock_anthropic_client
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_tool_calling_two_rounds_complete(
        self, mock_anthropic_class, sample_tool_definitions
    ):
        ...
        mock_tool_manager.execute_tool.assert_any_call(
            "search_course_content", query="round 1 query"
        )
        mock_tool_manager.execute_tool.assert_any_call(
            "search_course_content", query="round 2 query"
        )
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_early_termination_on_non_tool_response(
        self, mock_anthropic_class, sample_tool_definitions
    ):
        ...
        generator = AIGenerator("test-key", "test-model")
        result = generator.generate_response(
            "simple query", tools=sample_tool_definitions, tool_manager=Mock()
        )
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_termination_on_tool_failure(
        self, mock_anthropic_class, sample_tool_definitions
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_conversation_context_preserved(
        self, mock_anthropic_class, sample_tool_definitions
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_max_rounds_enforcement(
        self, mock_anthropic_class, sample_tool_definitions
    ):
    ...
    @patch("ai_generator.anthropic.Anthropic")
    def test_sequential_api_error_handling(
        self, mock_anthropic_class, sample_tool_definitions
    ):
